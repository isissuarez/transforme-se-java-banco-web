 ...
 * @author DELL
 */
@WebServlet(name = "inserircliente", urlPatterns = {"/inserir"})
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
          ...
          int status = clienteDAO.inserirCliente(cliente);
            ...
            if ( status > 0){
               out.print("<script>alert(\"Registro inserido com sucesso\";</script>");
               request.getRequestDispatcher("cliente.html").include(request, response);
            } 
           out.print("<script>alert(\"Erro ao inserir o cliente\"</script>");
               request.getRequestDispatcher("cliente.html").include(request, response); 
               out.close();
        }
